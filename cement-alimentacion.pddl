(define (domain cement-alimentacion)
  ...
  (:predicates
    ...
    (puzolana-s-ocupado ?m - molino) ; bandera individual por molino
  )
  (:functions
    ...
    :condition (and
      ...
      (over all (not (and (= ?m mc3) (alimentando yeso mc3))))
      (over all (not (and (= ?m mc3) (alimentando puzolana-s mc3))))
    ...
    :effect (and
      ...
      (at start (alimentando clinker ?m)) ; se marca que el molino alimenta clinker
      ...
      (at end (not (alimentando clinker ?m))) ; se borra marca de alimentacion
    )
  )
  ...
  (:durative-action alimentar-puzolana-s
    ...
    :condition (and
      ...
      (at start (not (puzolana-s-ocupado ?m)))
      (over all (not (and (= ?m mc1) (alimentando yeso mc1))))
      (over all (not (clinker-ocupado2 mc3)))
      (at start (not (alimentando-yeso-mc2 ?t)))
    )
    :effect (and
      ...
      (at start (alimentando puzolana-s ?m))
      (at start (puzolana-s-ocupado ?m))
      (at end (not (puzolana-s-ocupado ?m)))
      (at end (alimentado ?t puzolana-s))
      (at end (libre ?t))
      (at end (not (tolva-ocupada ?t puzolana-s)))
      (at end (not (alimentando puzolana-s ?m)))
    )
  )
