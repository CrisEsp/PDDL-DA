...
# Variables globales (asumiendo que estas existen en tu código)
level_fields = {}  # Diccionario para los campos de nivel de tolvas
feed_rate_fields = {}  # Diccionario para los campos de alimentación fresca
estado_molinos = {}  # Estado de los molinos (deberías definirlo)
estado_rutas = {}  # Estado de las rutas (deberías definirlo)
...
#     # Guardar el problema PDDL antes de ejecutar
#     try:
...
#         page.snack_bar = SnackBar(Text(f"❌ Error al guardar el problema PDDL: {e}"), open=True, duration=5000)
#         pddl_display.controls[0].value = f"Problema PDDL:\n{pddl_content}\n\nError: {e}"
#         refresh_cards(pddl_content, sistema, page)
#         page.update()
#         return
    
#     # Usar la clase PDDLExecutor modificada
#     try:
...
#                     plan_content = f.read()
#                 # Extraer solo las acciones del plan
#                 clean_plan = '\n'.join(line for line in plan_content.split('\n') if line.strip() and not line.startswith(';') and not any(s in line for s in ['Plan length:', 'Makespan:', 'Search time:', 'Total time:']))
#                 print(f"📜 Contenido para pddl_display:\n{clean_plan}")  # Depuración
#                 pddl_display.controls[0].value = f"Plan generado:\n{clean_plan}"
#                 print(f"📜 Asignado a pddl_display.controls[0].value: {pddl_display.controls[0].value}")  # Depuración adicional
...
#                 duration=5000
#             )
#             pddl_display.controls[0].value = f"Problema PDDL:\n{pddl_content}\n\nError: Falló la generación del plan"
...
#         # Buscar VS Code automáticamente (mantenido por compatibilidad)
#         self.vscode_path = None  # No se usa, pero se mantiene por compatibilidad
...
#         """Método mantenido por compatibilidad, no usado"""
...
#             # Paso 2: Ejecutar planificador TFD remoto
...
#         """Verifica archivos sin abrir VS Code"""
#         try:
...
#             # Verificar si la respuesta contiene una URL de verificación
...
#             # Reintentar hasta obtener el plan
...
#         """Captura el plan generado desde la respuesta del servicio remoto"""
#         try:
...
#         # Buscar la primera aparición de "Found new plan"
...
#         cleaned_lines = []
#         seen_actions = set()  # Para evitar duplicados
...
#                 line = re.sub(r'^\d+\.\d+:\s*', '', line)  # Remover numeración con timestamp
...
#         """Devuelve la ruta del plan más reciente"""
...
level_fields = {}  # Diccionario para los campos de nivel de tolvas
feed_rate_fields = {}  # Diccionario para los campos de alimentación fresca
estado_rutas = {}  # Estado de las rutas (deberías definirlo según tu modelo)

# Inicializar estado_molinos con valores de ejemplo
estado_molinos = {
    "mc1": {"activo": True},  # Ejemplo: estado de MC1
    "mc2": {"activo": True},  # Ejemplo: estado de MC2
    "mc3": {"activo": True}   # Ejemplo: estado de MC3
}

# Definir pddl_display y pddl_card globalmente
pddl_display = ft.ListView(
    controls=[
        ft.Text(
            "Presione 'Generar Problema PDDL' para ver el contenido.",
            color=ft.Colors.BLACK,
            size=14,
            expand=True,
            no_wrap=False
...
def main(page: ft.Page):
    ...
    try:
        ...
        # Agregar pddl_card y botón a la página
        page.add(pddl_card)
        page.add(
            ft.ElevatedButton(
                text="Generar Plan",
...
# Clase PDDLExecutor (sin cambios, incluida para referencia)
class PDDLExecutor:
    def __init__(self, domain_path, problem_path, workspace_path):
        self.domain_path = Path(domain_path.replace("\\", "/"))
        self.problem_path = Path(problem_path.replace("\\", "/"))
