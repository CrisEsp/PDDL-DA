...
#     def _select_best_plan(self, log_content):
#         print(f"🔍 Log completo recibido:\n{log_content[:500]}...")  # Truncado para debug
...
#         print(f"🔍 Número de bloques: {len(plan_blocks)}, Matches de métrica: {len(metric_matches)}")
        
#         for idx, (block, metric_match) in enumerate(zip(plan_blocks[1:], metric_matches)):
#             actions = "\n".join(line.strip() for line in block.splitlines() if re.match(r"^\d+\.\d+:", line))
#             if not actions:
#                 print(f"⚠️ Bloque {idx+1} vacío, saltando.")
#                 continue
            
#             try:
#                 metric_value = float(metric_match.group(1))
#             except (ValueError, IndexError):
#                 metric_value = float('inf')  # Si no parsea, ignora
            
#             print(f"🔍 Bloque {idx+1}: Métrica={metric_value}, Acciones iniciales:\n{actions[:200]}...")
            
#             # FIX: No saltar si métrica==0 (es óptimo para alimentaciones simples)
#             # Solo salta si inf o NaN
#             if not (isinstance(metric_value, (int, float)) and metric_value >= 0):
#                 print(f"⚠️ Métrica inválida {metric_value} en bloque {idx+1}, saltando.")
#                 continue
...
#                     "actions": actions,
#                     "metric": best_metric,
#                     "additional_info": f"\n\n# Info: Seleccionado bloque {idx+1} (primer plan válido)."
#                 }
#                 print(f"✅ Nuevo mejor: Métrica {best_metric} en bloque {idx+1}")
...
#         cleaned_actions = self._clean_plan(best_plan['actions'])
#         best_plan['actions'] = cleaned_actions
#         print(f"🔍 Plan final seleccionado (métrica {best_metric}):\n{cleaned_actions}")
...
#     # # En clase Molino: Modifica simular_consumo (subida gradual al 0, independiente por tolva)
#     # def simular_consumo(self, delta_horas: float):
#     #     """Simula consumo y alimentación heurística (subida gradual al 0 por tolva)."""
#     #     if not self.en_marcha:
#     #         return
#     #     for material, tolva in self.tolvas.items():
#     #         if material in self.ratios:
#     #             consumo_th = (self.alimentacion_fresca * self.ratios[material]) / 100 * self.rendimiento
#     #             tolva.actualizar_nivel_consumo(delta_horas, consumo_th)  # Baja a 0
                
#     #             # 🔥 Subida gradual independiente: Si <=0, alimenta poco a poco hasta >inef
#     #             if tolva.nivel_actual <= 0:
#     #                 feed_th = 0.5 * consumo_th  # Mitad del consumo como tasa de feed (~realista)
#     #                 delta_t_feed = feed_th * delta_horas
#     #                 delta_nivel_feed = (delta_t_feed / tolva.capacidad) * tolva.altura_max
#     #                 tolva.nivel_actual = min(tolva.altura_max, tolva.nivel_actual + delta_nivel_feed)
#     #                 if tolva.nivel_actual > tolva.nivel_ineficiente:  # Sale de modo feed al superar inef
#     #                     tolva.nivel_actual = tolva.nivel_ineficiente + random.uniform(0.5, 1.5)  # Buffer extra
#     #                 if "Yeso" in tolva.material and tolva.nivel_actual > 2.0:  # Pegado
#     #                     tolva.nivel_actual = 2.0
#     #                 print(f"📈 {self.nombre}-{material}: Feed gradual +{delta_nivel_feed:.3f}m → {tolva.nivel_actual:.3f} (de 0)")
#     #             # Si aún bajo pero >0 y <inef, continúa feed gradual (independiente)
#     #             elif tolva.nivel_actual < tolva.nivel_ineficiente:
#     #                 feed_th = 0.5 * consumo_th
#     #                 delta_t_feed = feed_th * delta_horas
#     #                 delta_nivel_feed = (delta_t_feed / tolva.capacidad) * tolva.altura_max
#     #                 tolva.nivel_actual = min(tolva.altura_max, tolva.nivel_actual + delta_nivel_feed)
#     #                 print(f"📈 {self.nombre}-{material}: Feed bajo inef +{delta_nivel_feed:.3f}m → {tolva.nivel_actual:.3f}")
                
#     #             # Dinámica random: Solo si >inef (evita sobre-feed)
#     #             elif random.random() < 0.3:
#     #                 delta_t = random.uniform(5, 20)
#     #                 delta_nivel = (delta_t / tolva.capacidad) * tolva.altura_max
#     #                 tolva.nivel_actual = min(tolva.altura_max, tolva.nivel_actual + delta_nivel)
#     #                 print(f"📈 {self.nombre}-{material}: +{delta_t:.1f}t → {tolva.nivel_actual:.3f}")

#     # # En simular_consumo_y_actualizar: UI sync post-feed (doble update para web)
#     # def simular_consumo_y_actualizar(page, sistema: SistemaAlimentacion):
#     #     delta_horas = 2.0  # Aumenta a 2h/tick para hits 0 más rápidos (ajusta a 1 si lento)
#     #     sistema.simular_consumo_global(delta_horas)
...
#     #     # UI In-Place post-feed (barras suben si feed)
#     #     session_state = page.session_state
#     #     for molino in [sistema.mc1, sistema.mc2, sistema.mc3]:
#     #         for material, tolva in molino.tolvas.items():
#     #             field_key = f"{molino.nombre}_{material}"
...
#     #             # ProgressBar: Recalcula post-feed, clamp seguro
#     #             if progress_key in session_state['progress_bars']:
#     #                 max_level = tolva.altura_max if molino.nombre != "MC3" else 100
#     #                 progress = max(0.01, min(1.0, tolva.nivel_actual / max_level))  # Min 0.01 para evitar pegado en 0
#     #                 bar_color = ft.Colors.GREEN_ACCENT_700 if progress >= 0.5 else ft.Colors.YELLOW_700 if progress >= 0.2 else ft.Colors.RED_700
#     #                 session_state['progress_bars'][progress_key].value = progress
#     #                 session_state['progress_bars'][progress_key].color = bar_color
...
#     #     page.update()  # Primera: Sync cambios
#     #     time.sleep(0.1)  # Micro-delay para web render (opcional, quita si no)
#     #     page.update()  # Segunda: Force visual subida
...
def generate_plan(page, sistema):
    session_state = page.session_state
    first_plan = session_state.get('first_plan', True)
    ...
    try:
        ...
        if success and gantt_img and best_plan_obj:
            ...
            if first_plan:
                session_state['first_plan'] = False
        else:
            print("❌ Falló la generación del plan o del diagrama de Gantt")
            session_state['pddl_display'].controls.clear()
            session_state['pddl_display'].controls.append(
        ...
        for molino in [sistema.mc1, sistema.mc2, sistema.mc3]:
            for material, tolva in molino.tolvas.items():
                ...
                if field_key in session_state['level_fields']:
                    ...
                    # unit = "%" if molino.nombre == "MC3" else "m"
                    # session_state['level_fields'][field_key].value = f"{round(tolva.nivel_actual, 2)} {unit}"
                    session_state['level_fields'][field_key].value = f"{round(tolva.nivel_actual, 2)} "
