...
class Molino:
    ...
    def simular_consumo(self, delta_horas: float):
        """Simula consumo y alimentación heurística con reset/progresión cuando llega a 0."""
        if not self.en_marcha:
            return
        for material, tolva in self.tolvas.items():
            if material in self.ratios:
                # Consumo ajustado por material (más lento para yeso)
                factor_ajuste = 1.0 if "clinker" in material else 0.5 if "yeso" in material else 1.0  # Yeso más lento
                consumo_th = (self.alimentacion_fresca * self.ratios[material] / 100 * self.rendimiento) * factor_ajuste
                tolva.actualizar_nivel_consumo(delta_horas, consumo_th)  # Baja progresiva

                # Reset/Feeding: Si <=0, inicia subida gradual (progresiva, no salto)
                if tolva.nivel_actual <= 0:
                    # Feeding rate heurístico: 2-5 t/h simulados (lento para visual)
                    feeding_rate = random.uniform(2, 5)  # t/h
                    delta_feed_t = feeding_rate * delta_horas * 0.5  # Mitad tick para progresión
                    tolva.actualizar_nivel_alimentacion(delta_feed_t)
                    if tolva.nivel_actual > tolva.nivel_ineficiente + 1:  # Límite buffer
                        tolva.nivel_actual = tolva.nivel_ineficiente + 1
                    if "yeso" in material.lower() and tolva.nivel_actual > 2.0:
                        tolva.nivel_actual = 2.0
                    print(f"🔄 {self.nombre}-{material}: Feeding progresivo +{delta_feed_t:.2f}t → {tolva.nivel_actual:.2f} (barra sube gradual)")

                # Subida aleatoria opcional (independiente)
                elif random.random() < 0.2:  # Menos frecuente
                    delta_t = random.uniform(2, 10)  # Menor para progresión
...
async def main(page: ft.Page):
    ...
    def simular_consumo_y_actualizar(page, sistema: SistemaAlimentacion):
        delta_horas = 2.0  # Balance: visible pero no saturado (ajusta a 1.5 para más lento)
        ...
        # UI Update: In-place, con clamp explícito para subidas
        session_state = page.session_state
        for molino in [sistema.mc1, sistema.mc2, sistema.mc3]:
            for material, tolva in molino.tolvas.items():
                field_key = f"{molino.nombre}_{material}"
                ...
                # Nivel (solo numérico, sin unit en value para evitar parse errors)
                if field_key in session_state['level_fields']:
                    session_state['level_fields'][field_key].value = str(round(tolva.nivel_actual, 2))
                
                # ProgressBar: Min 0.01 para subidas visibles post-reset
                if progress_key in session_state['progress_bars']:
                    max_level = tolva.altura_max if molino.nombre != "MC3" else 100
                    progress = max(0.01, min(1.0, tolva.nivel_actual / max_level))  # Min visible >0
                    bar_color = ft.Colors.GREEN_ACCENT_700 if progress >= 0.5 else ft.Colors.YELLOW_700 if progress >= 0.2 else ft.Colors.RED_700
                    session_state['progress_bars'][progress_key].value = progress
                    session_state['progress_bars'][progress_key].color = bar_color
                
                # T.V (agrega Text si falta, update value)
                tiempo = molino.tiempo_vaciado(material)
                tiempo_key = f"{molino.nombre}_{material}_tiempo"
                if tiempo_key not in session_state:
                    session_state[tiempo_key] = ft.Text(f"{tiempo:.2f} h", size=14, color=ft.Colors.WHITE)
                else:
                    session_state[tiempo_key].value = f"{tiempo:.2f} h"
        
        page.update()  # Seguro en main thread
        
        # PDDL: Trigger heurístico si crítico
    ...
    async def periodic_update():
        while True:
            ...
            if page.session_state.get('sistema'):
                simular_consumo_y_actualizar(page, page.session_state['sistema'])  # Directo, sin run_thread
            else:
